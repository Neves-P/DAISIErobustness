  ...
  for (l in seq_len(nrow(parameters))) {
    ...
    if ((prop_rep_over_20_spec / replicates) < 0.95 ||
      ...
      for (n_reps in 1:replicates) {
        ...
        stt_last_row_constant_1 <- nrow(constant_simulations_2[[n_reps]][[1]][[1]]$stt_all[, 5])
        num_spec_constant_1 <- constant_simulations_2[[n_reps]][[1]][[1]]$stt_all[stt_last_row_constant_1, 5]
