plot_error_jitters <- function(error_metrics_list,
  ...
  p <- ggplot2::ggplot(data = data, ggplot2::aes(y = value, x =  "1", shape = key)) +
    ggplot2::theme_bw() +
    ggplot2::geom_jitter(height = 0, width = 1) +
    # ggplot2::geom_violin(na.rm = TRUE, show.legend = FALSE, scale = "count") +
